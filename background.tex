...
for rt scheduling when rt task group scheduling is not enabled.
In fact, this \texttt{task\_rq} can be used even in case rt and cfs task group
scheduling are enabled. It just that when such task group schedling are set,
normally the information in each element in the scheduling route is more
important than simply returning a runqueue. 
